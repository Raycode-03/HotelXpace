...
export default function HotelsPage({ query, type, price, limit }: HotelsPageProps) {
  ...
  const { data: hotelData, isLoading } = useQuery({
    ...
    queryFn: () => hotelApi.hotelCardList(query, activeType, activePrice, limit),
  ...
  return (
    <div className='pt-20 md:pt-14 lg:pt-16 top-0 flex flex-col md:flex-row h-screen bg-primary/90'>
      <div className={`${mapOpen ? 'hidden' : 'flex'} md:flex flex-col w-full md:w-[40%] overflow-y-auto`}>
        <HotelList
      ...
      <div className={`${mapOpen ? 'fixed inset-0 z-40 flex flex-col' : 'hidden'} md:relative md:flex md:w-[60%]  md:h-auto`}>
        <HotelMap onClose={() => setMapOpen(false)}
          hotels={hotels}
          selectedHotelId={selectedHotelId}
          onSelectHotel={setSelectedHotelId}
        />
